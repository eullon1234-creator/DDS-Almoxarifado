...
import { Settings, Lock, Eye, EyeOff, X, Key, ShieldCheck, Terminal } from 'lucide-react';
import { useSettings } from '@/lib/SettingsContext';
import type { Settings as SettingsType } from '@/lib/SettingsContext';
...
export default function ConfigPanel() {
    ...
    const toggleItems: Array<{ key: keyof SettingsType; label: string; desc: string }> = [
        { key: 'enableAI', label: 'Ativar IA (Texto)', desc: 'Permite gerar temas via IA neural.' },
        { key: 'enableImageGen', label: 'Ativar Geracao de Imagens', desc: 'Permite criar imagens via IA externa.' },
        { key: 'showImages', label: 'Exibir Imagens no Painel', desc: 'Mostra ou oculta imagens geradas no painel.' },
        { key: 'verboseLogging', label: 'Logs Detalhados', desc: 'Ativa logs de depuracao no console.' },
    ];
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="p-4 bg-cyber-black/5 border border-cyber-blue/20 rounded-sm space-y-3">
                                <h3 className="text-cyber-cyan font-bold text-[10px] uppercase tracking-widest">Recursos Avancados</h3>
                                <div className="space-y-3">
                                            ...
                                            <label className="inline-flex items-center cursor-pointer">
                                                <input
                                                    ...
                                                    onChange={() => {
                                                        ...
                                                        showSuccessMessage('CONFIGURACOES ATUALIZADAS');
                                                    }}
                                                />
                                                <div className={`w-10 h-6 flex items-center rounded-full p-1 transition-all ${settings[item.key] ? 'bg-cyber-cyan' : 'bg-cyber-blue/20'}`}>
                                                    <div className={`bg-white w-4 h-4 rounded-full transform transition-transform ${settings[item.key] ? 'translate-x-4' : 'translate-x-0'}`}></div>
                                                </div>
                                            </label>
                                        </div>
                                    ))}
